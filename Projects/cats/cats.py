...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    ps_selected = [p for p in paragraphs if select(p)]
    return ps_selected[k] if k < len(ps_selected) else ""
    # END PROBLEM 1
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    similar_words = [
        w for w in word_list if diff_function(typed_word, w, limit) <= limit
    ]
    if not similar_words:
        return typed_word
    return min(similar_words, key=lambda w: diff_function(typed_word, w, limit))
    # END PROBLEM 5
...
def get_time(times, player_num, word_index):
    ...
    assert word_index < len(times[0]), (
        f"word_index {word_index} outside of 0 to {num_words - 1}"
    )
    assert player_num < len(times), (
