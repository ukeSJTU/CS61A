...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place = self.place
        cur_range = 0
        while cur_range < self.lower_bound and not place.is_hive:
            place = place.entrance
            cur_range += 1
        while cur_range <= self.upper_bound and not place.is_hive:
            if len(place.bees) > 0:
                break
            place = place.entrance
            cur_range += 1
        if place.is_hive or cur_range > self.upper_bound:
            return None
        else:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # apply reflected damage to all bees in the same place
        bees = place.bees[:]
        for bee in bees:
